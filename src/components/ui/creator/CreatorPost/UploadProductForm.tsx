...
export interface UploadProductDialogProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function UploadProductDialog({
        ...
        success: (res) => {
          if (res?.success) {
            ...
            setTimeout(() => {
              ...
              resetForm();
              router.push(`/shop/${res?.data?._id}`)
              onClose();
            }, 1500);
          } else {
            toast.error(res?.message || 'Failed to upload product');
          }
          return res?.message || 'Product uploaded successfully';
        },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4 overflow-y-auto selection:bg-emerald-500/30">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="px-6 py-5 border-b border-[#2a2a35] flex items-center justify-between sticky top-0 bg-[#0d0d12] z-10 rounded-t-4xl shrink-0">
            <h2 className="text-lg font-black tracking-tight text-white flex items-center gap-2">
              Upload New Product <ShoppingBag size={18} className="text-emerald-500" />
            </h2>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="p-6 overflow-y-auto space-y-7 custom-scrollbar flex-1">
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white/70 group-hover:text-white transition-colors">
                ...
                <p className="text-[11px] font-bold uppercase tracking-widest">Select Product Image</p>
              </div>
              <input
                type="file"
                ref={fileInputRef}
            ...
            >
              {isSubmitting ? (
                ...
                <span className="flex items-center justify-center gap-1.5">
                  Publish Product <ChevronRight size={16} />
                </span>
              )}
            </button>
          </div>
